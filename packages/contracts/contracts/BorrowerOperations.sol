...
contract BorrowerOperations is LiquityBase, Ownable, CheckContract, IBorrowerOperations {
    ...
    */
    ...
    function _triggerBorrowingFee(ITroveManager _troveManager, ILUSDToken _lusdToken, uint _LUSDAmount, uint _maxFeePercentage) internal returns (uint) {
        ...
        uint divider = 2;
        uint fee = LUSDFee.div(divider);
        uint tostaking = LUSDFee.sub(fee);
        //send fee to feemanager
        _lusdToken.mint(lqtyStakingAddress, tostaking);
        _lusdToken.mint(feemanager, fee);
